...
EMV.irarma <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="log")
{
  ...
  if(steps!=0){
    ...
    for(i in 1:steps) 
    {
      ...
      y_prev_imedian[n+i] <-ir.q(rep(0.5,1),lambda=lambdaf[i],mu=muf[i])
      y_prev_imean[n+i]<-(1-lambdaf[i])*muf[i]
      y_prev_mu[n+i]<-muf[i]
    ...
    ########################################################################
    yr_prev_imedian<-y_prev_imedian
    yr_prev_imean<-y_prev_imean
    yr_prev_mu<-y_prev_mu
    z$rollingforecast_imedian<-z$forecast_imedian
    z$rollingforecast_imean<-z$forecast_imean
    z$rollingforecast_mu<-z$forecast_mu
