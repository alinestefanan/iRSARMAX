  ...
  
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      ...
      eta2[i] <- X[i,1]%*%as.matrix(beta0) + X[i,2:ncol(X)]%*%as.matrix(beta)+ sum(ar_par*(ynew[i-ar_ind]) ) 
  ...
  score <- function(z) 
  {
    ...
    beta <- z[2:(k+1)]
    phi = z[(k+2):(p1+k+1)] 
    lambda0 <- z[(p1+k+2)] # lambda parameter
    lambda1 <- z[(p1+k+3)]
    ...
    score<-c(Ubeta0,Ubeta,Uphi,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,p1+k), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0p,KB0lambda0,KB0lambda1),
      cbind(KBB0,KBB,KBp,KBlambda0,KBlambda1),
      cbind(KpB0,KpB,Kpp,Kplambda0,Kplambda1),
      cbind(Klambda0B0,Klambda0B,Klambda0p,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B0,Klambda1B,Klambda1p,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept",1:k,ar, "lambda0 estimator", "lambda1 estimator")
  ...
  measures.fitted=function(yfit,residual){
    ...
    monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf = (p1))
