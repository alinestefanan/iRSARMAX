  ...
  
{
  ...
  score <- function(z) 
  {
    ...
    theta =z[(k+2):(q1+k+1)]
    PHI = z[(q1+k+2):(q1+P1+k+1)]
    lambda0 <- z[(P1+q1+k+2)] # lambda parameter
    lambda1 <- z[(P1+q1+k+3)]
    ...
    score<-c(Ubeta0,Ubeta,Utheta, UPHI,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,q1+P1+k), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  lambda0 <-coef[(k+P1+q1+2)] # lambda parameter
  lambda1 <-coef[(k+P1+q1+3)]
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0q,KB0P,KB0lambda0,KB0lambda1),
      cbind(KBB0,KBB,KBq,KBP,KBlambda0,KBlambda1),
      cbind(KqB0,KqB,Kqq,KqP,Kqlambda0,Kqlambda1),
      cbind(KPB0,KPB,KPq,KPP,KPlambda0,KPlambda1),
      cbind(Klambda0B0,Klambda0B,Klambda0q,Klambda0P,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B0,Klambda1B,Klambda1q,Klambda1P,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept",1:k,ma, AR,"lambda0 estimator", "lambda1 estimator")
  ...
  measures.fitted=function(yfit,residual){
    ...
    monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf = (q1+P1))
