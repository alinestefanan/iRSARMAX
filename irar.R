  ...
  
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      ...
      eta2[i] <- X[i,]%*%as.matrix(beta0) + sum(phi*(ynew[i-ar]) )
  ...
  for(i in (m+1):n)
  {
    ...
    etahat2[i] <- X[i,]%*%as.matrix(beta0) + sum(phi*(ynew[i-ar]) ) 
  ...
  if(steps!=0){
    ...
    for(i in 1:steps) 
    {
      ...
      eta2_prev_imedian[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(z$phi*(y_prev_imedian[n+i-ar]) ) 
      muf_imedian[i]<-linkinv(eta2_prev_imedian[n+i]) 
      y_prev_imedian[n+i] <-ir.q(rep(0.5,1),lambda=lambdaf_imedian[i],mu=muf_imedian[i])
      
      eta2_prev_imean[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(z$phi*(y_prev_imean[n+i-ar]) ) 
      muf_imean[i]<-linkinv(eta2_prev_imean[n+i]) 
      y_prev_imean[n+i]<-(1-lambdaf_imean[i])*muf_imean[i]
      
      eta2_prev_mu[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(z$phi*(y_prev_mu[n+i-ar]) ) 
    ...
    for(i in 1:steps)
    {
      ...
      eta2_rw[n+i] <- X_prev[n+i,1]*z$beta0 + sum(z$phi*(y[n+i-ar]) )
