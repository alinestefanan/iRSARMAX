...
EMV.irarma <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="log")
{
  ...
  ir.q<-Vectorize(function(u,lambda,mu)
  {
    ...
    for (i in 1:n){
      ...
      sample.error<- tryCatch(u[i]<=lambda[i], error = function(e) return("error")) 
      if(sample.error[1] ==  "error")
      {z$RMC=1
  ...
  loglik <- function(z) 
  {
    ...
    lambda0 <- z[(p1+P1+q1+k+2)] 
    lambda1 <- z[(p1+P1+q1+k+3)]
    ...
    for(i in (m+1):n)
    {
      eta1[i] <- X[i,1]%*%as.matrix(lambda0) + sum(lambda1*(ynew[i-1]))
      eta2[i] <- X[i,1]%*%as.matrix(beta0) + X[i,2:ncol(X)]%*%as.matrix(beta)+ sum(ar_par*(ynew[i-ar_ind]) ) - sum(ma_par*error[i-ma_ind])
  ...
  score <- function(z) 
  {
    ...
    theta =z[(p1+k+2):(p1+q1+k+1)]
    PHI = z[(p1+q1+k+2):(p1+q1+P1+k+1)]
    lambda0 <- z[(p1+P1+q1+k+2)] # lambda parameter
    lambda1 <- z[(p1+P1+q1+k+3)]
    ...
    for(i in (m+1):n)
    {
      ...
      deta.dbeta[i,]<-  B[(i-m),] +  ma_par%*%(mu.eta(eta2[i-ma_ind])*deta.dbeta[i-ma_ind,])
    ...
    score<-c(Ubeta0,Ubeta,Uphi,Utheta, UPHI,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,p1+q1+P1+k), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  z$conv <- opt$conv
  coef <- (opt$par)[1:(k+p1+q1+P1+3)]
  z$coeff <- coef
  
  beta0 <-coef[1] #intercept
  beta <-coef[2:(k+1)] #covariates
  phi<-coef[(k+2):(k+p1+1)] #ar
  theta <-coef[(k+p1+2):(k+p1+q1+1)] #ma
  PHI <-coef[(k+p1+q1+2):(k+p1+q1+P1+1)] #AR
  lambda0 <-coef[(k+p1+P1+q1+2)] # lambda parameter
  lambda1 <-coef[(k+p1+P1+q1+3)]
  ...
  for(i in (m+1):n)
  {
    ...
    etahat2[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind]) ) - sum(ma_par*errorhat[i-ma_ind])
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    KB0B0 <- -(t(mM0)%*%mV2%*%(mT2^2)%*%mM0 )
    KB0B <- -(t(mM0)%*%mV2%*%(mT2^2)%*%mM )
    KB0p <- -(t(mM0)%*%mV2%*%(mT2^2)%*%pp )
    KB0q <- -(t(mM0)%*%mV2%*%(mT2^2)%*%qq)
    KB0P <- -(t(mM0)%*%mV2%*%(mT2^2)%*%PP)
    ...
    if(length(KBB)==1){
      KBB <- -(t(mM)%*%mV2%*%(mT2^2)%*%mM )
    }else{
      for(j in 1:k){
        for(i in 1:k){
          KBB[i,j] <- -(t(mM[,i])%*%mV2%*%(mT2^2)%*%mM[,j])
        }
      }
    }
    KBp=matrix(rep(NA,k*p1),ncol=p1)
    if(length(KBp)==1){
      KBp <- -(t(mM)%*%mV2%*%(mT2^2)%*%pp )
    }else{
      for(j in 1:p1){
        for(i in 1:k){
          KBp[i,j] <- -(t(mM[,i])%*%mV2%*%(mT2^2)%*%pp[,j])
        }
      }
    }
    KBq=matrix(rep(NA,k*q1),ncol=q1)
    if(length(KBq)==1){
      KBq <- -(t(mM)%*%mV2%*%(mT2^2)%*%qq)
    }else{
      for(j in 1:q1){
        for(i in 1:k){
          KBq[i,j] <- -(t(mM[,i])%*%mV2%*%(mT2^2)%*%qq[,j] )
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0p,KB0q,KB0P,KB0lambda0,KB0lambda1),
      cbind(KBB0,KBB,KBp,KBq,KBP,KBlambda0,KBlambda1),
      cbind(KpB0,KpB,Kpp,Kpq,KpP,Kplambda0,Kplambda1),
      cbind(KqB0,KqB,Kqp,Kqq,KqP,Kqlambda0,Kqlambda1),
      cbind(KPB0,KPB,KPp,KPq,KPP,KPlambda0,KPlambda1),
      cbind(Klambda0B0,Klambda0B,Klambda0p,Klambda0q,Klambda0P,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B0,Klambda1B,Klambda1p,Klambda1q,Klambda1P,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept",1:k,ar, ma, AR,  "lambda0 estimator", "lambda1 estimator")
  ...
  measures.fitted=function(yfit,residual){
    ...
    monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf = (p1+q1+P1))
  ...
  if(steps!=0){
    eta1_prev_imean<-eta1_prev_mu<-eta1_prev_imedian <- c(ynew,rep(NA,steps))
    ...
    lambdaf_imedian<-lambdaf_imean<-lambdaf_mu<-NA
    ntotal<-n+steps
    X_prev <- matrix(c(rep(1,ntotal),exvar), nrow=ntotal, ncol=(k+1),byrow = F)
    
    for(i in 1:steps) 
    {
      eta1_prev_imedian[n+i] <- X_prev[n+i,1]%*%as.matrix(z$lambda0) + sum(z$lambda1*(y_prev_imedian[n+i-1]))
      lambdaf_imedian[i] <-exp(eta1_prev_imedian[n+i])/(exp(eta1_prev_imedian[n+i])+1)
      
      eta1_prev_imean[n+i] <- X_prev[n+i,1]%*%as.matrix(z$lambda0) + sum(z$lambda1*(y_prev_imean[n+i-1]))
      lambdaf_imean[i] <-exp(eta1_prev_imean[n+i])/(exp(eta1_prev_imean[n+i])+1)
      
      eta1_prev_mu[n+i] <- X_prev[n+i,1]%*%as.matrix(z$lambda0) + sum(z$lambda1*(y_prev_mu[n+i-1]))
      lambdaf_mu[i] <-exp(eta1_prev_mu[n+i])/(exp(eta1_prev_mu[n+i])+1)
      
      eta2_prev_imedian[n+i] <- X_prev[n+i,1]%*%as.matrix(z$beta0)+ X_prev[n+i,2:ncol(X_prev)]%*%as.matrix(z$beta) + sum(ar_par*(y_prev_imedian[n+i-ar_ind]) ) - sum(ma_par*errorhat[n+i-ma_ind])
      muf_imedian[i]<-linkinv(eta2_prev_imedian[n+i]) 
      y_prev_imedian[n+i] <-ir.q(rep(0.5,1),lambda=lambdaf_imedian[i],mu=muf_imedian[i])
      
      eta2_prev_imean[n+i] <- X_prev[n+i,1]%*%as.matrix(z$beta0)+ X_prev[n+i,2:ncol(X_prev)]%*%as.matrix(z$beta) + sum(ar_par*(y_prev_imean[n+i-ar_ind]) ) - sum(ma_par*errorhat[n+i-ma_ind])
      muf_imean[i]<-linkinv(eta2_prev_imean[n+i]) 
      y_prev_imean[n+i]<-(1-lambdaf_imean[i])*muf_imean[i]
      
      eta2_prev_mu[n+i] <- X_prev[n+i,1]%*%as.matrix(z$beta0)+ X_prev[n+i,2:ncol(X_prev)]%*%as.matrix(z$beta) + sum(ar_par*(y_prev_mu[n+i-ar_ind]) ) - sum(ma_par*errorhat[n+i-ma_ind])
    ...
    for(i in 1:steps)
    {
      eta1_rw[n+i] <- X_prev[n+i,1]%*%as.matrix(z$lambda0) + sum(z$lambda1*(y[n+i-1]))
      lambdarwf[i] <-exp(eta1_rw[n+i])/(exp(eta1_rw[n+i])+1)
      eta2_rw[n+i] <- X_prev[n+i,1]*z$beta0 + X_prev[n+i,2:ncol(X_prev)]%*%as.matrix(z$beta) + sum(ar_par*(y[n+i-ar_ind]) ) - sum(ma_par*errorhat[n+i-ma_ind])
      murwf[i] <- linkinv(eta2_rw[n+i])
      yr_prev_imedian[n+i] <-ir.q(rep(0.5,1),lambda=lambdarwf[i],mu=murwf[i])
      yr_prev_mu[n+i] <-murwf[i]
      yr_prev_imean[n+i] <-(1-lambdarwf[i])*murwf[i]
    }
  ...
  if(graph==T)
  {
    ...
    pdf("obs_v_fit_imedian.pdf",width=5, height=4)### abre no navegador google chrome só
    {
      par(mfrow=c(1,1))
      par(mar=c(2.8, 2.7, 1, 1)) 
      par(mgp=c(1.7, 0.45, 0))
