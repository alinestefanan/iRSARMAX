  ...
  
{
  ...
  score <- function(z) 
  {
    ...
    phi = z[(k+2):(p1+k+1)] 
    PHI = z[(p1+2):(p1+P1+1)]
    lambda0 <- z[(p1+P1+2)] # lambda parameter
    lambda1 <- z[(p1+P1+3)]
    ...
    score<-c(Ubeta0,Uphi, UPHI,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,p1+P1), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  z$conv <- opt$conv
  coef <- (opt$par)[1:(p1+P1+3)]
  z$coeff <- coef
  
  beta0 <-coef[1] #intercept 
  phi<-coef[(2):(p1+1)] #ar
  PHI <-coef[(p1+2):(p1+P1+1)] #AR
  lambda0 <-coef[(p1+P1+2)] # lambda parameter
  lambda1 <-coef[(p1+P1+3)]
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    K <- rbind(
      cbind(KBB,KBp,KBP,KBlambda0,KBlambda1),
      cbind(KpB,Kpp,KpP,Kplambda0,Kplambda1),
      cbind(KPB,KPp,KPP,KPlambda0,KPlambda1),
      cbind(Klambda0B,Klambda0p,Klambda0P,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B,Klambda1p,Klambda1P,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept",ar, AR, "lambda0 estimator", "lambda1 estimator")
  ...
  measures.fitted=function(yfit,residual){
    ...
    monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf = (p1+P1))
