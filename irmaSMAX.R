  ...
  
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      ...
      eta2[i] <- X[i,1]%*%as.matrix(beta0) + X[i,2:ncol(X)]%*%as.matrix(beta)- sum(ma_par*error[i-ma_ind])
  ...
  score <- function(z) 
  {
    ...
    theta =z[(k+2):(q1+k+1)]
    THETA = z[(q1+k+2):(q1+Q1+k+1)]
    lambda0 <- z[(q1+Q1+k+2)] # lambda parameter
    lambda1 <- z[(q1+Q1+k+3)]
    ...
    score<-c(Ubeta0,Ubeta,Utheta,UTHETA,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,q1+Q1+k), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  z$conv <- opt$conv
  coef <- (opt$par)[1:(k+q1+Q1+3)]
  z$coeff <- coef
  
  beta0 <-coef[1] #intercept
  beta <-coef[2:(k+1)] #covariates
  theta <-coef[(k+2):(k+q1+1)] #ma
  THETA <-coef[(k+q1+2):(k+q1+Q1+1)] #MA
  lambda0 <-coef[(k+q1+Q1+2)] # lambda parameter
  lambda1 <-coef[(k+q1+Q1+3)]
  ...
  for(i in (m+1):n)
  {
    ...
    etahat2[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) - sum(ma_par*errorhat[i-ma_ind])
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0q,KB0Q,KB0lambda0,KB0lambda1),
      cbind(KBB0,KBB,KBq,KBQ,KBlambda0,KBlambda1),
      cbind(KqB0,KqB,Kqq,KqQ,Kqlambda0,Kqlambda1),
      cbind(KQB0,KQB,KQq,KQQ,KQlambda0,KQlambda1),
      cbind(Klambda0B0,Klambda0B,Klambda0q,Klambda0Q,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B0,Klambda1B,Klambda1q,Klambda1Q,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept",1:k,ma, MA, "lambda0 estimator", "lambda1 estimator")
  ...
  measures.fitted=function(yfit,residual){
    ...
    monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf = (q1+Q1))
  ...
  if(steps!=0){
    ...
    for(i in 1:steps) 
    {
      ...
      eta2_prev[n+i] <- X_prev[n+i,1]%*%as.matrix(z$beta0)+ X_prev[n+i,2:ncol(X_prev)]%*%as.matrix(z$beta) - sum(ma_par*errorhat[n+i-ma_ind])
      muf[i]<-linkinv(eta2_prev[n+i]) 
      y_prev_imedian[n+i] <-ir.q(rep(0.5,1),lambda=lambdaf_imedian[i],mu=muf[i])
      y_prev_imean[n+i]<-(1-lambdaf_imean[i])*muf[i]
      y_prev_mu[n+i]<-muf[i]
    ...
    for(i in 1:steps)
    {
      ...
      yr_prev_imedian[n+i] <-ir.q(rep(0.5,1),lambda=lambdarwf[i],mu=muf[i])
      yr_prev_mu[n+i] <-muf[i]
      yr_prev_imean[n+i] <-(1-lambdarwf[i])*muf[i]
    }
