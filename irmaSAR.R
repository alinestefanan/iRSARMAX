  ...
  
{
  ...
  score <- function(z) 
  {
    ...
    theta =z[(k+2):(q1+k+1)]
    PHI = z[(q1+2):(q1+P1+1)]
    lambda0 <- z[(P1+q1+2)] # lambda parameter
    lambda1 <- z[(P1+q1+3)]
    ...
    score<-c(Ubeta0,Utheta, UPHI,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,q1+P1), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  z$conv <- opt$conv
  coef <- (opt$par)[1:(q1+P1+3)]
  z$coeff <- coef
  
  beta0 <-coef[1] #intercept 
  theta <-coef[(2):(q1+1)] #ma
  PHI <-coef[(q1+2):(q1+P1+1)] #AR
  lambda0 <-coef[(P1+q1+2)] # lambda parameter
  lambda1 <-coef[(P1+q1+3)]
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    K <- rbind(
      cbind(KBB,KBq,KBP,KBlambda0,KBlambda1),
      cbind(KqB,Kqq,KqP,Kqlambda0,Kqlambda1),
      cbind(KPB,KPq,KPP,KPlambda0,KPlambda1),
      cbind(Klambda0B,Klambda0q,Klambda0P,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B,Klambda1q,Klambda1P,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept", ma, AR, "lambda0 estimator", "lambda1 estimator")
