  ...
  
{
 ...
 #  irq.error<- tryCatch(ir.q(u=rep(0.5,length(lambdahat)),lambda=lambdahat,mu=muhat))
 #  if(irq.error[1] == "error")
 #  {z$RMC=1
 #  print("irq error")
 # # print("lambdahat");print(lambdahat)
 ...
 #  }
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    for(i in (m+1):n)
    {
      ...
      #   for(a in 1:k)
      #   {
      #     deta.dbetaphi[a,b,i] <- 0 + ma_par%*%(mu.eta(etahat2[i-ma_ind])*deta.dbetaphi[a,b,i-ma_ind])
      #       
      #   }
      # }
      # for(b in 1:q1)
