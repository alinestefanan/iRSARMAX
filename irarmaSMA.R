  ...
  
{
  ...
  score <- function(z) 
  {
    ...
    score<-c(Ubeta0,Uphi,Utheta,UTHETA,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,p1+q1+Q1), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    K <- rbind(
      cbind(KBB,KBp,KBq,KBQ,KBlambda0,KBlambda1),
      cbind(KpB,Kpp,Kpq,KpQ,Kplambda0,Kplambda1),
      cbind(KqB,Kqp,Kqq,KqQ,Kqlambda0,Kqlambda1),
      cbind(KQB,KQp,KQq,KQQ,KQlambda0,KQlambda1),
      cbind(Klambda0B,Klambda0p,Klambda0q,Klambda0Q,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B,Klambda1p,Klambda1q,Klambda1Q,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept",ar, ma,  MA, "lambda0 estimator", "lambda1 estimator")
