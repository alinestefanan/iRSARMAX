  ...
  
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      ...
      eta2[i] <- X[i,]%*%as.matrix(beta0) + sum(ar_par*(ynew[i-ar_ind]) ) 
  ...
  score <- function(z) 
  {
    beta0 <- z[1:(k+1)]
    PHI = z[(2):(P1+1)]
    lambda0 <- z[(P1+2)] # lambda parameter
    lambda1 <- z[(P1+3)]
    ...
    score<-c(Ubeta0,UPHI,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,P1), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  PHI <-coef[(2):(P1+1)] #AR
  lambda0 <-coef[(P1+2)] # lambda parameter
  lambda1 <-coef[(P1+3)]
  ...
  for(i in (m+1):n)
  {
    ...
    etahat2[i] <- X[i,]%*%as.matrix(beta0) + sum(ar_par*(ynew[i-ar_ind]) )
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    K <- rbind(
      cbind(KBB,KBP,KBlambda0,KBlambda1),
      cbind(KPB,KPP,KPlambda0,KPlambda1),
      cbind(Klambda0B,Klambda0P,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B,Klambda1P,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept", AR, "lambda0 estimator", "lambda1 estimator")
  ...
  if(steps!=0){
    ...
    for(i in 1:steps) 
    {
      ...
      eta2_prev_imedian[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(ar_par*(y_prev_imedian[n+i-ar_ind]) )
      muf_imedian[i]<-linkinv(eta2_prev_imedian[n+i]) 
      y_prev_imedian[n+i] <-ir.q(rep(0.5,1),lambda=lambdaf_imedian[i],mu=muf_imedian[i])
      
      eta2_prev_imean[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(ar_par*(y_prev_imean[n+i-ar_ind]) )
      muf_imean[i]<-linkinv(eta2_prev_imean[n+i]) 
      y_prev_imean[n+i]<-(1-lambdaf_imean[i])*muf_imean[i]
      
      eta2_prev_mu[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(ar_par*(y_prev_mu[n+i-ar_ind]) )
    ...
    for(i in 1:steps)
    {
      ...
      eta2_rw[n+i] <- X_prev[n+i,1]*z$beta0 + sum(ar_par*(y[n+i-ar_ind]) ) 
