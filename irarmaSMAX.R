  ...
  
{
  ...
  loglik <- function(z) 
  {
    ...
    lambda0 <- z[(p1+q1+Q1+k+2)] 
    lambda1 <- z[(p1+q1+Q1+k+3)]
  ...
  score <- function(z) 
  {
    ...
    theta =z[(p1+k+2):(p1+q1+k+1)]
    THETA = z[(p1+q1+k+2):(p1+q1+Q1+k+1)]
    lambda0 <- z[(p1+q1+Q1+k+2)] # lambda parameter
    lambda1 <- z[(p1+q1+Q1+k+3)]
    ...
    score<-c(Ubeta0,Ubeta,Uphi,Utheta, UTHETA,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,p1+q1+Q1+k), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  THETA <-coef[(k+p1+q1+2):(k+p1+q1+Q1+1)] #MA
  lambda0 <-coef[(k+p1+q1+Q1+2)] # lambda parameter
  lambda1 <-coef[(k+p1+q1+Q1+3)]
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0p,KB0q,KB0Q,KB0lambda0,KB0lambda1),
      cbind(KBB0,KBB,KBp,KBq,KBQ,KBlambda0,KBlambda1),
      cbind(KpB0,KpB,Kpp,Kpq,KpQ,Kplambda0,Kplambda1),
      cbind(KqB0,KqB,Kqp,Kqq,KqQ,Kqlambda0,Kqlambda1),
      cbind(KQB0,KQB,KQp,KQq,KQQ,KQlambda0,KQlambda1),
      cbind(Klambda0B0,Klambda0B,Klambda0p,Klambda0q,Klambda0Q,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B0,Klambda1B,Klambda1p,Klambda1q,Klambda1Q,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept",1:k,ar, ma, MA, "lambda0 estimator", "lambda1 estimator")
  ...
  measures.fitted=function(yfit,residual){
    ...
    monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf = (p1+q1+Q1))
