...
EMV.irarma <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="log")
{
  ...
  score <- function(z) 
  {
    ...
    beta <- z[2:(k+1)]
    theta =z[(k+2):(q1+k+1)]
    lambda0 <- z[(q1+k+2)] # lambda parameter
    lambda1 <- z[(q1+k+3)]
    ...
    score<-c(Ubeta0,Ubeta,Utheta,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,q1+k), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0q,KB0lambda0,KB0lambda1),
      cbind(KBB0,KBB,KBq,KBlambda0,KBlambda1),
      cbind(KqB0,KqB,Kqq,Kqlambda0,Kqlambda1),
      cbind(Klambda0B0,Klambda0B,Klambda0q,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B0,Klambda1B,Klambda1q,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept",1:k, ma, "lambda0 estimator", "lambda1 estimator")
