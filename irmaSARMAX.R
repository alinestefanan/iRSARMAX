  ...
  
{
  ...
  score <- function(z) 
  {
    ...
    score<-c(Ubeta0,Ubeta,Utheta, UPHI,UTHETA,Ulambda0,Ulambda1)
    return(score)
  }#fim score
  
  y1<-y[(m+1):n] 
  
  reg <- c(0,rep(0,q1+P1+Q1+k), length(y1[which(y1==0)])/length(y1),0) 
  z=c()
  ...
  z$conv <- opt$conv
  coef <- (opt$par)[1:(k+q1+P1+Q1+3)]
  z$coeff <- coef
  
  beta0 <-coef[1] #intercept
  beta <-coef[2:(k+1)] #covariates
  theta <-coef[(k+2):(k+q1+1)] #ma
  PHI <-coef[(k+q1+2):(k+q1+P1+1)] #AR
  ...
  Fisher.inf<-function(y1,muhat,lambdahat)
  {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0q,KB0P,KB0Q,KB0lambda0,KB0lambda1),
      cbind(KBB0,KBB,KBq,KBP,KBQ,KBlambda0,KBlambda1),
      cbind(KqB0,KqB,Kqq,KqP,KqQ,Kqlambda0,Kqlambda1),
      cbind(KPB0,KPB,KPq,KPP,KPQ,KPlambda0,KPlambda1),
      cbind(KQB0,KQB,KQq,KQP,KQQ,KQlambda0,KQlambda1),
      cbind(Klambda0B0,Klambda0B,Klambda0q,Klambda0P,Klambda0Q,Klambda0lambda0,Klambda0lambda1),
      cbind(Klambda1B0,Klambda1B,Klambda1q,Klambda1P,Klambda1Q,Klambda1lambda0,Klambda1lambda1)
    )
    return(K)
  }
  ...
  first_col<-c("intercept",1:k,ma, AR, MA, "lambda0 estimator", "lambda1 estimator")
  ...
  measures.fitted=function(yfit,residual){
    ...
    monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf = (q1+P1+Q1))
