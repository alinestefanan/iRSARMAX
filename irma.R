  ...
  
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      ...
      eta2[i] <- X[i,]%*%as.matrix(beta0) - sum(theta*error[i-ma])
  ...
  for(i in (m+1):n)
  {
    ...
    etahat2[i] <- X[i,]%*%as.matrix(beta0) - sum(theta*errorhat[i-ma])
  ...
  if(steps!=0){
    ...
    for(i in 1:steps) 
    {
      ...
      eta2_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) - sum(z$theta*errorhat[n+i-ma])
